...
abstract contract KernelERC1271Test is KernelTestBase {
    ...
    function test_erc1271_root_enable_mode_returns_invalid() external unitTest erc1271Test {
        bytes memory signature = abi.encodePacked(bytes1(0x08), bytes1(0x00), bytes("malformed enable payload"));
        assertEq(kernel.isValidSignature(keccak256("root enable disabled"), signature), ERC1271_INVALID);
    }

    function test_erc1271_truncated_enable_mode_returns_invalid() external unitTest erc1271Test {
        assertEq(kernel.isValidSignature(keccak256("truncated enable disabled"), hex"08"), ERC1271_INVALID);
    }

    function test_erc1271_permission_enable_mode_returns_invalid() external unitTest erc1271Test {
        bytes memory signature = abi.encodePacked(bytes1(0x08), bytes1(0x02), permissionId, bytes("payload"));
        assertEq(kernel.isValidSignature(keccak256("permission enable disabled"), signature), ERC1271_INVALID);
    }

    function test_erc1271_replayable_enable_mode_returns_invalid() external unitTest erc1271Test {
        bytes memory signature = abi.encodePacked(bytes1(0x09), bytes1(0x01), address(newValidator), bytes("payload"));
        assertEq(kernel.isValidSignature(keccak256("replayable enable disabled"), signature), ERC1271_INVALID);
    }
