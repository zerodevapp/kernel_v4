    ...
    function _verifyStatelessSignature(
        ...
        if (vType == VALIDATION_TYPE_VALIDATOR) {
            ...
            for (uint256 i; i < packages.length; i++) {
                Install calldata pkg = packages[i];
                // Restrict matching to policy / signer modules. Otherwise a package of a
                // different module type (e.g. selector type 3 or hook type 4) whose internalData
                // happens to start with `pId` would be enrolled into the permission's signature
                // chain.
                if (
                    PermissionId.wrap(bytes4(pkg.internalData)) == pId
                        && (pkg.moduleType == MODULE_TYPE_POLICY || pkg.moduleType == MODULE_TYPE_SIGNER)
                ) {
