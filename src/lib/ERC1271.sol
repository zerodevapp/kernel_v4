...
abstract contract ERC1271 is EIP712 {
        ...
        returns (bool result)
    {
        ...
        (hash, signature, t) = _erc1271ProcessTypedDataSign(hash, signature, t, false);
        if (t == uint256(0)) hash = _hashTypedData(hash); // `PersonalSign` workflow.
        result = _erc1271IsValidSignatureNowCalldata(hash, signature);
    }

    /// @dev Replayable variant of the nested EIP-712 workflow (omits chainId from the domain).
    /// forge-lint: disable-next-line(mixed-case-function)
        ...
        returns (bool result)
    {
        ...
        (hash, signature, t) = _erc1271ProcessTypedDataSign(hash, signature, t, true);
        if (t == uint256(0)) hash = _hashTypedData(hash); // `PersonalSign` workflow.
        result = _erc1271IsValidSignatureNowCalldata(hash, signature);
    }

    /// @dev Shared helper for processing the TypedDataSign / PersonalSign workflow.
    ///      Handles signature parsing, contents extraction, type encoding, and hash computation.
    /// @param hash The original hash to validate.
    /// @param signature The signature with appended TypedDataSign data.
    /// @param t Memory pointer to the domain fields buffer (set to 0 if PersonalSign fallback triggered).
    /// @param replayable If true, omits chainId from the TypedDataSign type encoding and uses 0xc0 hashStruct size.
    /// @return The (potentially recomputed) hash, truncated signature, and updated t (0 = PersonalSign).
    function _erc1271ProcessTypedDataSign(bytes32 hash, bytes calldata signature, uint256 t, bool replayable)
        internal
        pure
        returns (bytes32, bytes calldata, uint256)
    {
        /// @solidity memory-safe-assembly
        assembly {
            ...
            for {} 1 {} {
                ...
                mstore(p, " contents,string name,string") // Store the shared prefix of the encoding.
                // Branch: chain-specific includes "uint256 chainId,", replayable omits it.
                switch replayable
                case 0 {
                ...
                // hashStruct size: 0xe0 with chainId (7 words), 0xc0 without (6 words).
                let hashSize := add(0xc0, mul(iszero(replayable), 0x20))
                mstore(0x40, keccak256(t, hashSize)) // `hashStruct(typedDataSign)`.
